...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/updates/EntryStreamContainer.tsx"
      to: "src/lib/updates.ts"
      via: "UpdateEntry type import"
      pattern: "import.*UpdateEntry.*from.*@/lib/updates"
---
...
**CRITICAL i18n imports -- use ONLY these hooks from `@/lib/i18n`:**
- `useLocale` returns `{ locale, setLocale }` -- use `const { locale } = useLocale()`
- `useTranslations` returns the `t` function directly -- use `const t = useTranslations()`
- Import path: `import { useLocale, useTranslations } from '@/lib/i18n'`
- Do NOT use `useLanguage`, `useLang`, or any other hook name. They do not exist.

**CRITICAL type imports -- use ONLY these paths:**
- `NowEntry` from `@/lib/now`
- `UpdateEntry` from `@/lib/updates` (NOT `@/types/updates` -- that path does not exist)
...
- Import `UpdateEntry` from `@/lib/updates` (NOT `@/types/updates`)
...
Do NOT use generic CSS variables like `--bg`, `--surface`, `--border`, `--accent`, `--text-primary`, `--text-muted` -- those do not exist in this project. The correct Tailwind class names use the `color-*` prefix tokens, e.g. `bg-base-900`, `text-text-primary`, `border-base-800`, `text-amber-500`.
  </action>
  <verify>
**IMPORTANT:** Plan 10-01 must have been executed first. The `updates.*` translation keys must exist in `messages/en.json` before running TypeScript checks, because `TranslationKey` is derived from en.json at build time. If Plan 10-01 has not been applied, `t('updates.pageTitle')` and similar calls will fail type checking.
...
Additionally verify:
- All three components import from `@/lib/i18n` (NOT `@/hooks/useLanguage` or any other path)
- EntryStreamContainer imports `UpdateEntry` from `@/lib/updates` (NOT `@/types/updates`)
- No CSS classes reference non-existent variables (no `bg-surface`, `text-accent`, `border-border`, etc.)
  </verify>
  <done>
...
<verification>
**Pre-condition:** Plan 10-01 must be complete before running these checks. The `updates.*` keys in `messages/en.json` are required for `TranslationKey` type generation -- without them, `t('updates.pageTitle')` will not compile.
...
- All components use `useLocale`/`useTranslations` from `@/lib/i18n` (grep confirms no `useLanguage` references)
- All components use `UpdateEntry` from `@/lib/updates` (grep confirms no `@/types/updates` references)
</verification>
...
- All i18n hooks imported from `@/lib/i18n`, not from any other path
- All types imported from their correct source modules
</success_criteria>
