...
2. Structure (top to bottom within the clickable area):
   ...
   - **Excerpt:** The entry's `summary` field displayed as muted text (text-text-muted, text-sm, line-clamp-2). Use the summary field directly -- do NOT attempt to truncate the HTML body.
...
**UpdatesPageContent.tsx** -- No structural changes needed. The component already passes entries to EntryStreamContainer. Just verify it still works after the EntryStreamContainer replacement. If it imports anything that changed, update accordingly.
  ...
  <done>Entry list shows formatted date + tag chip + title + excerpt per entry, entries are separated by thin dividers, hover lifts with cream background and inverted text, clicking navigates to /updates/[slug], list staggers in on page load</done>
</task>
...
- Summary field used for excerpt (not HTML body truncation)
</verification>

<success_criteria>
- Entries render as flat list items with thin dividers
- Each entry: meta row (date + dot + tag chip), Fraunces 20px title, muted excerpt
